...
The Power BI solution presented is designed to analyze and visualize timesheet data, focusing on key performance metrics related to employee hours, sales amounts, and project management. The primary purpose of this solution appears to be to provide insights into resource allocation, project profitability, and overall operational efficiency within an organization. By leveraging data from various sources, including timesheets and project details, the solution aims to facilitate informed decision-making and enhance project management capabilities.

The data model is structured around several key tables, including `vw_Timesheet`, `tbl_Project`, and `DimDate`, which collectively support a range of measures and calculations. Important metrics such as total hours worked, sales amounts, average hourly rates, and project margins are calculated using DAX measures. The solution also incorporates various calculated columns to derive additional insights, such as employee status and project classifications. The reports generated from this model likely focus on visualizing these metrics through pivot tables and charts, allowing users to drill down into specific projects, employees, and time periods for a comprehensive analysis of performance and resource utilization.
...
- **Qualify**: (int64) Represents a qualifying metric, summarized by sum.
...
**Data Source:**  
The data is imported from an Excel workbook located at a specified SharePoint URL, with the relevant data extracted from a sheet named "Lookup". The table is processed to remove null and whitespace entries, promote headers, and ensure the correct data types are assigned.
...
**Description:**  
The `tbl_Project` table contains detailed information about various projects, including their identifiers, descriptions, statuses, financial details, and key dates. This table is essential for tracking project-related data within the reporting framework.
...
This table serves as a foundational dataset for reporting and analysis of employee timesheet data within the organization.
...
This formula first creates a variable `ActiveEmployees` that filters and summarizes the `vw_missing_timesheet` table to identify employees with negative missing hours and a minimum active employee count of 1. It then returns the sum of the missing hours for these active employees. 
...
**Purpose:**  
The `Msr_ActiveContacts` measure calculates the number of active contacts (employees) within a specified date range based on their contract start and end dates.
...
This formula sums the total sales amount from the `vw_Timesheet` table and divides it by the total hours worked, also sourced from the same table. 
...
**Purpose:**  
The `Msr_HoursPercentage` measure calculates the percentage of actual hours worked relative to the total hours across selected categories, projects, and employees.
...
- It then divides the sum of actual hours worked by this `TotalValue`, returning 0 if the `TotalValue` is zero. 
...
**Calculation Logic:**  
The measure uses the `SUMX` function to iterate over a summarized table created by `SUMMARIZE`, which groups the `vw_Timesheet` data by `ProjectProfile` and `Client`. For each group, it retrieves the selected value of `SalesPriceBaseCurrency` from the `tbl_Project` table and sums these values.
...
This formula counts the distinct employee names in the `vw_Timesheet` table while ignoring any filters on the table and only considering those employees whose contract status is marked as "Valid". 
...
**Calculation Logic:**  
The measure uses the `SUMX` function to iterate over a distinct list of employee names from the `vw_Timesheet` table, summing the maximum hours per week for each employee. The formula is as follows:
...
**Purpose:**  
The `trackedDiff2` measure calculates the difference between the total hours tracked and the maximum hours per week for each employee, while removing filters on the `HoursPerWeek` column and retaining context for the `EmployeeID`.
...
This formula first retrieves the total hours tracked using the `TotalHoursTracked` measure. It then calculates the maximum hours per week for the distinct values of `HoursPerWeek`, while ignoring any filters on that column and keeping the context for the specific employee. The final result is the difference between these two values.
...
2. It checks if the number of active contacts (`Msr_ActiveContacts`) is not blank. If true, it further checks if `_TotalHours` is not blank.
...
- No specific filters defined at the page/report level.
...
### Data Fields Used
- Unit (from `vw_Timesheet.MAIN_UNIT`)
...
### Data Fields Used
- **Data Fields Used:**
  - From `config.json`: Not specified.
  - From `query.json`: Not specified.
  - From `dataTransforms.json`: 
    - `tbl_Project.DevoteamPillar`
    - `vw_Timesheet.MAIN_UNIT`

### Visual-Level Filters
- **Visual-Level Filters:** Not specified.

### Applicable Page/Report-Level Filters
- **Applicable Page/Report-Level Filters:** Not specified.
...
### Data Fields Used
- **From Projections in config.json:** Not specified.
- **From Selects in query.json:** Not specified.
- **From dataTransforms.json:** 
  - `tbl_Project.DevoteamPillar`
