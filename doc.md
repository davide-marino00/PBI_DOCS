...
The data model includes several key components, such as the `vw_Timesheet` and `tbl_Project` tables, which contain essential information about employee hours, sales amounts, project statuses, and financial details. Key measures calculated within the model include total hours worked, cumulative sales, project margins, and realization percentages. The solution also incorporates various dimensions, such as time (via the `DimDate` table), to facilitate time-based analysis. The visualizations are likely to include pivot tables that summarize data by project, employee, and time periods, allowing users to drill down into specific areas of interest for deeper insights. Overall, this Power BI solution serves as a robust tool for tracking and optimizing project and employee performance.
...
**Description:**  
The `dim_date` table serves as a date dimension, providing various attributes related to dates for analytical purposes. It is designed to facilitate time-based analysis in reporting and data visualization.
...
**Description:**  
The `tbl_project` table is designed to store detailed information about various projects, including their attributes, statuses, and financial details.
...
**Calculation Logic:**  
The measure is calculated using the DAX function `WEEKNUM`, which takes the current date (`TODAY()`) as an argument. The formula is as follows:
...
**Purpose:**  
The `Dax_EmpCount_MissingTS` measure calculates the count of active employees who have missing timesheet hours, specifically those with negative missing hours and a minimum active employee count of one.
...
This formula first summarizes the `vw_missing_timesheet` table to calculate the total missing hours and the minimum active employee count for each employee. It then filters this summarized data to include only those employees who are active (with a minimum count of one) and have negative missing hours. Finally, it counts the number of rows in the filtered result, representing the count of employees with missing timesheet hours. 
...
This formula computes the maximum value of `ContractHours` and subtracts the maximum value of `Hours_`, providing a measure of the shortfall in hours worked compared to what is contracted.
...
This formula checks if the sum of hours is blank. If it is, it returns 0; otherwise, it returns the sum of hours tracked. 
...
**Purpose:**  
The `TotalHoursTrackedMissing` measure is designed to evaluate the total hours tracked in the timesheet, providing a specific output when no hours are recorded.
...
**Purpose:**  
The `UTI_TOTALHOURS` measure calculates the total hours worked by employees on projects that qualify as billable.
...
**Purpose:**  
The `UniqueEmployeesPerUnit` measure calculates the count of unique employees per unit based on their weekly records in the `vw_missing_timesheet` table, specifically focusing on those with an incomplete flag.

**Calculation Logic:**  
The measure uses the `SUMX` function in conjunction with `SUMMARIZE` to group the data by week, employee name, and main unit. It retrieves the maximum value of the `IncompleteFlag` for each group and sums these values to determine the total count of unique employees with incomplete records.
...
### Data Fields Used
- **Field/Measure:** `tbl_Project.DevoteamPillar`
