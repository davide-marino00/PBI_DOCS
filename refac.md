...
The Power BI solution presented is designed to analyze and visualize timesheet data, focusing on key performance metrics related to employee hours, sales amounts, and project management. The primary purpose of this solution appears to be to provide insights into resource allocation, project profitability, and overall operational efficiency within an organization. By leveraging data from various sources, including timesheets and project details, the solution enables users to track performance against budgets, monitor employee contributions, and assess project statuses.

The data model is structured around several key tables, including `vw_Timesheet`, `tbl_Project`, and `DimDate`, which collectively support a range of measures and calculations. Important metrics include total hours worked, sales amounts, average hourly rates, and project margins. The solution employs various DAX measures to calculate cumulative sales, budget utilization, and realization percentages, allowing for dynamic reporting and analysis. Visual components such as pivot tables and charts facilitate the exploration of this data, enabling users to drill down into specific projects, employees, and time periods for a comprehensive view of performance metrics.
...
**Purpose:** The DimDate table serves as a date dimension, providing various attributes related to dates for analytical purposes in Power BI.

**Key Columns:**
- **DateKey**: An integer representing the unique identifier for each date.
- **ShortDate**: A dateTime field representing the actual date.
- **DayName**: A string indicating the name of the day (e.g., Monday).
- **DayNumberOfWeek**: An integer representing the day number within the week (1-7).
- **DayNumberOfMonth**: An integer representing the day number within the month.
- **DayNumberOfYear**: An integer representing the day number within the year.
- **WeekNumberOfYear**: An integer representing the week number within the year.
- **MonthName**: A string indicating the name of the month.
- **MonthNumberOfYear**: An integer representing the month number (1-12).
- **CalendarQuarter**: A string indicating the quarter of the year.
- **CalendarYear**: An integer representing the year.
- **CurrentYearFlag**: An integer flag indicating if the year is the current year (1 for yes, 0 for no).
...
**Key Columns:**
- **Hours_nd_Percentage**: Represents the calculated hours and percentage values, sourced from `[Value1]`.
- **Hours_nd_Percentage Fields**: A hidden column that groups related data, sourced from `[Value2]`.
- **Hours_nd_Percentage Order**: A hidden column that determines the order of the entries, sourced from `[Value3]`, and is summarized by sum.

**Partitioning and Source Information:**  
The table is calculated and imported, utilizing total actual hours and hours percentage from the `vw_Timesheet` view. 

**Annotations:**  
- Summarization is set to automatic for all columns.
- The table has an associated PBI_Id for identification.

#### Table: `lkp_Code`

### Table: lkp_Code

**Purpose:** The `lkp_Code` table serves as a lookup table, likely providing reference codes and associated attributes for data analysis.

**Key Columns:**
- **Code**: (string) Represents the unique code identifier.
- **Qualify**: (int64) A numeric value used for summarization, indicating a qualification metric.
- **Sort**: (string) A field used for sorting purposes.

**Data Source:** The table is imported from an Excel workbook located at a specified SharePoint URL, with data transformation steps applied to ensure proper formatting and type assignment.
...
**Key Columns:**
- **EmployeeId**: A unique identifier for each employee (data type: string).
- **EmployeeName**: The name of the employee (data type: string).

**Data Source:** The table is imported from an Excel workbook located at a specified SharePoint URL, which contains a sheet named "Lookup". The data is processed to remove null and whitespace entries, promote headers, and change column types to text.
...
**Description:**  
The `tbl_Project` table contains detailed information about various projects, including their identifiers, descriptions, financial details, and status. It serves as a central repository for project-related data, facilitating reporting and analysis.
...
- **TotalContractSum** (decimal): Total financial value of the contract associated with the project.
...
**Key Columns:**
- `week_`: Represents the week of the timesheet.
...
**Description:**  
The `vw_Timesheet` table is designed to aggregate and analyze timesheet data, providing insights into employee hours, project statuses, and contract details.
...
This table serves as a foundational dataset for reporting and analysis related to employee timesheets and project management.
...
**Calculation Logic:**  
The measure uses the `CALCULATE` function to modify the filter context and counts the distinct `EmployeeName` from the `vw_Timesheet` table where the `Approved` column is set to `FALSE()`. 
...
This formula first creates a variable `ActiveEmployees` that filters a summarized table of `vw_missing_timesheet` for employees with at least one active status and negative missing hours. It then calculates the total missing hours for these active employees using the `SUMX` function. 
...
1. It defines two variables, `_MaxDate` and `_MinDate`, which capture the maximum and minimum dates from the `DimDate[ShortDate]` column.
2. It creates a table `_StartsBeforeMonthEnd` that includes employee IDs from `vw_Timesheet` where the `ShortDate` is less than or equal to `_MaxDate`.
3. It creates another table `_notEndAfterMonthEnd` that includes employee IDs where the `ShortDate` is greater than or equal to `_MinDate` or is blank, utilizing the `USERELATIONSHIP` function to relate `DimDate[ShortDate]` to `vw_Timesheet[ContractEndDate]`.
4. Finally, it returns the count of rows that are present in both `_notEndAfterMonthEnd` and `_StartsBeforeMonthEnd` using the `INTERSECT` function.

This measure effectively identifies employees whose contracts are active within the specified date range.
...
**Purpose:**  
The `Msr_Budget` measure calculates the current budget value based on the selected week and dynamically retrieves the most recent available budget if the current budget is not available.
...
**Calculation Logic:**  
The measure sums the `SalesAmount` from the `vw_Timesheet` table, applying a filter to include only those dates that are less than or equal to the maximum date in the current context. The formula used is:
...
This formula first computes the total hours from the `vw_Timesheet` table while considering all selected values for `GroupCat`, `Project`, and `EmployeeName`. It then divides the sum of hours for the current context by this total, returning a percentage. If the total value is zero, it returns zero to avoid division errors. 
...
- **[TotalContractedHours]**: This measure calculates the total contracted hours by summing the maximum hours per week for each employee.
- The `DIVIDE` function is used to safely perform the division, returning 0 if the denominator ([TotalContractedHours]) is zero.
...
**Purpose:**  
The `TotalHoursTrackedMissing` measure is designed to evaluate the total hours tracked in the timesheet. It specifically checks if the sum of hours is blank and returns a specific message if it is.
...
This formula checks if the sum of hours from the `vw_Timesheet` table is blank. If it is blank, it returns the string "Empty"; otherwise, it returns the total sum of hours tracked. 
...
This formula computes the difference by taking the total hours tracked and subtracting the maximum distinct hours per week for the employee, ensuring that the calculation is not affected by filters on `HoursPerWeek` while retaining the context of the specific employee.
...
**Purpose:**  
The `UniqueEmployeesPerUnit` measure calculates the count of unique employees per unit based on their incomplete status within a specified week.

**Calculation Logic:**  
The measure uses the `SUMX` function in conjunction with `SUMMARIZE` to group data from the `vw_missing_timesheet` table by week, employee name, and main unit. It evaluates the maximum value of the `IncompleteFlag` for each group, effectively counting unique employees who have an incomplete status.
...
1. It defines two variables:
   - `_TotalBillableHours`: This retrieves the total billable hours using the measure `[UTI_TotalBillableHours]`.
   - `_TotalHours`: This retrieves the total hours using the measure `[UTI_TOTALHOURS]`.

2. The measure checks if the number of active contacts (`[Msr_ActiveContacts]`) is not blank. If true, it further checks if the total hours (`[UTI_TOTALHOURS]`) is not blank.
...
#### Calculated Column:`DimDate'['CurrentYearFlag`

### DimDate[CurrentYearFlag]

**Purpose:**  
The `CurrentYearFlag` column is designed to indicate whether a date in the `DimDate` table falls within the current year.
...
**Purpose:**  
The `BillablePrj` column is designed to determine whether a project is billable based on specific criteria related to sales price, project profile code, and project name.
...
#### Calculated Column:`vw_missing_timesheet'['CC_ActiveEmployees`
...
**Purpose:**  
The `GroupCat` column categorizes projects into "Billable" or "Other Unbillable" based on the project information and whether the project is billable.
...
#### Calculated Column:`vw_Timesheet'['Approved_With_V_Z`

### Calculated Column: `vw_Timesheet[Approved_With_V_Z]`

**Purpose:**  
This column determines whether a timesheet entry is approved or falls under specific timesheet codes ("V" or "Z"). It outputs a binary value indicating approval status.
...
#### Calculated Column:`vw_Timesheet'['BillableDep`

### Column: BillableDep

**Purpose:**  
The `BillableDep` column is designed to determine whether a department is billable based on the related unit's billable status.
...
#### Calculated Column:`vw_Timesheet'['IncompleteFlag`

### IncompleteFlag

The `IncompleteFlag` column is designed to indicate whether a timesheet entry is incomplete based on certain criteria. 

The DAX calculation for this column is as follows:

```dax
MAX(vw_missing_timesheet[IncompleteFlag])
```

#### Calculated Column:`vw_Timesheet'['MAIN_UNIT`

### Column: `MAIN_UNIT`

**Purpose:**  
The `MAIN_UNIT` column is designed to retrieve the unit associated with a record from a related lookup table. If no unit is found, it defaults to "Unknown".
...
#### Calculated Column:`vw_Timesheet'['YearWeek`

### Calculated Column: vw_Timesheet['YearWeek']

**Purpose:**  
The `YearWeek` column is likely intended to represent a combination of the year and week number for timesheet entries, facilitating time-based analysis and reporting.

**DAX Calculation Logic:**  
``` 
YearWeek = CONCATENATE(DimDate.CalendarYear, DimDate.WeekNumberOfYear)
```
