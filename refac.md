...
- `CurrentYearFlag` (int64): A flag indicating if the year is the current year.
- `DateKey` (int64): A unique key representing each date.
- `DayName` (string): The name of the day (e.g., Monday, Tuesday).
- `DayNumberOfMonth` (int64): The numerical representation of the day in the month.
- `DayNumberOfWeek` (int64): The numerical representation of the day in the week.
- `DayNumberOfYear` (int64): The numerical representation of the day in the year.
- `MonthName` (string): The name of the month (e.g., January, February).
- `MonthNumberOfYear` (int64): The numerical representation of the month in the year.
...
- `Code` (string): Represents a unique code used for lookup purposes.
- `Qualify` (int64): Represents a qualification value associated with the code, which is summed in aggregations.
- `Sort` (string): Represents a sorting value for the codes, likely used for ordering in reports or visualizations.
...
The `lkp_Project` table appears to serve as a lookup table for project-related data, containing information about projects, their qualification status, billing amounts, and associated groups. This is inferred from the column names and their data types, which suggest a focus on project management metrics.

##### Regular Columns

- `Billing` (int64): Represents the billing amount related to the project, also summarized by sum.
- `Group` (string): Represents the group associated with the project.
- `Project` (string): Represents the name or identifier of a project.
- `Qualify` (int64): Represents a qualification metric associated with the project, summarized by sum.
...
The `vw_missing_timesheet` table appears to serve the purpose of tracking and analyzing missing timesheet entries for employees, including details about their hours worked, contract status, and various metrics related to their timesheet submissions.
...
- `ContractStatusToday` (string): Indicates the current status of the employee's contract.
...
- `ShortDate` (dateTime): Represents a short format date of the timesheet entry.
...
The `vw_Timesheet` table appears to serve the purpose of tracking timesheet data for employees, including hours worked, project details, and contract statuses. It includes various measures for calculating hours, contract statuses, and financial metrics related to employee work.

##### Regular Columns

- `Approved` (boolean): Indicates if the timesheet is approved.
- `Approved_With_V_Z` (Unknown): DAX expression to check approval status with specific codes.
- `BillableDep` (Unknown): DAX expression to determine if the department is billable.
- `BillablePrj` (Unknown): DAX expression to determine if the project is billable.
...
- `Currency` (string): Currency used for the amounts.
...
- `Employee_WeeklyHours` (Unknown): DAX expression to concatenate employee name with weekly hours.
...
- `GroupCat` (Unknown): DAX expression to categorize the group.
- `Hours` (double): Total hours worked.
- `HoursType` (string): Type of hours worked.
- `HoursTypeCode` (string): Code for the type of hours.
- `HoursperWeek` (int64): Number of hours worked per week.
- `Index` (int64): Index for the timesheet entry.
- `IngestDatetime` (dateTime): Date and time when the data was ingested.
- `InternalPMID` (string): ID of the internal project manager.
- `InternalPMKey` (int64): Identifier for the internal project manager.
- `IsContractActive` (Unknown): DAX expression to check if the contract is active.
- `JobTitle` (string): Job title of the employee.
- `Join_Key` (string): Key for joining with other tables.
- `MAIN_UNIT` (Unknown): DAX expression to determine the main unit.
...
- `MonthNumber` (Unknown): DAX expression to extract the month number from the timesheet date.
...
- `QualifyPrj` (Unknown): DAX expression to determine if the project qualifies.
- `Rejected` (boolean): Indicates if the timesheet is rejected.
- `ReportReady` (boolean): Indicates if the report is ready.
- `ReportingEntity` (Unknown): DAX expression to determine the reporting entity.
...
- `YearWeek` (string): Year and week information.
- `cc_Employer` (Unknown): DAX expression to categorize the employer.
...
**Purpose:**  
The `ContractStatus2` measure evaluates the status of a contract based on the active status determined by the `ContractStatusMeasure` and returns the difference in hours if the contract is active. If the contract is not active, it returns "Not Active".
...
**Context:**  
- It relies on the `ContractStatusMeasure`, which checks if the contract is active based on the `ContractEndDate`.
- It uses the `HoursDifference` measure to calculate the difference between total hours worked and the maximum hours per week if the contract is active.
...
**Purpose:**  
The `ContractStatusMeasure` is designed to determine the current status of a contract based on its end date. It categorizes contracts as either "Active" or "Not Active" depending on whether the contract end date is blank or greater than today's date.
...
**Explanation:**  
- The measure checks the maximum value of the `ContractEndDate` from the `vw_Timesheet` table.
- If the maximum end date is blank or is a future date (greater than today), it returns "Active".
...
This measure leverages the `CALCULATE` function to modify the filter context, counting only those employees whose `MissingHours` are less than zero.
...
This measure is essential for reporting and analysis related to employee participation and activity within the context of timesheet data.
...
- **Key Variables:**
  - `MissingHours`: The difference between total hours worked and contracted hours.
...
**Purpose:**  
The `Dax_EmpCount_RReady` measure calculates the distinct count of employees whose reports are not marked as ready. This measure is useful for identifying employees who have pending reports, which can help in tracking reporting compliance and performance.
...
**Purpose:**  
The `Dax_MissingHours` measure calculates the total number of missing hours for active employees who have reported negative missing hours in a given context (year and week). It identifies employees with insufficient hours logged compared to their contracted hours.
...
**Context:**  
- The measure operates on the `vw_missing_timesheet` table, summarizing data by employee, year, week, unit, and subunit.
...
**Purpose:**  
The `DISTINCT_COUNT_EMP` measure calculates the total number of unique employees based on their names from the `vw_Timesheet` table. This measure is useful for reporting and analysis where understanding the distinct count of employees is necessary, such as in workforce analytics or project staffing assessments.
...
**Format:**  
- The result is formatted as a whole number (`formatString: 0`). 

This measure provides a straightforward count of unique employee entries, ensuring that each employee is only counted once, regardless of how many times they appear in the dataset.
...
- The final result is the sum of these maximum contract hours across all employees and weeks, providing the total expected contracted hours for the week. 
...
**Annotations:**  
- **PBI_FormatHint:** `{"isGeneralNumber":true}` - This indicates that the measure is formatted as a general number.
...
- **[Dax_EmpCount_MissingTS]**: Count of employees with missing timesheet entries, calculated by filtering the summarized data based on specific conditions.
...
**Purpose:**  
The `MissingHoursOut` measure calculates the difference between the maximum contracted hours and the maximum hours worked for employees in the `vw_missing_timesheet` table. This measure helps identify the total hours that are missing or unaccounted for in relation to the contracted hours.
...
**Context:**  
- `vw_missing_timesheet[ContractHours]`: Represents the total hours an employee is contracted to work.
- `vw_missing_timesheet[Hours_]`: Represents the actual hours worked by the employee.

This measure is useful for assessing discrepancies between expected and actual work hours, aiding in workforce management and reporting.
...
**Purpose:**  
The `Msr_ActiveContacts` measure calculates the number of active contacts (employees) based on their contract dates relative to a specified date range. It identifies employees whose contracts are active within the selected date context.
...
**Explanation:**
1. **Variables:**
   - `_MaxDate`: The maximum date from the `DimDate` table within the current context.
   - `_MinDate`: The minimum date from the `DimDate` table within the current context.
   - `_StartsBeforeMonthEnd`: A table of unique `EmployeeID`s whose contract start dates are on or before `_MaxDate`.
   - `_notEndAfterMonthEnd`: A table of unique `EmployeeID`s whose contract end dates are on or after `_MinDate` or are blank, utilizing a user-defined relationship to the `ContractEndDate`.

2. **Return Value:**  
   The measure returns the count of employees that are both active (not ended after the month end) and started before the month end by calculating the intersection of the two sets of employee IDs.
...
**Purpose:**  
The `MSR_AVG_HOURLY_RATE` measure calculates the average hourly rate based on the total sales amount and total hours worked. This measure is useful for assessing the revenue generated per hour of labor, providing insights into profitability and efficiency.
...
- It assumes that both `SalesAmount` and `Hours` are non-zero to avoid division errors.
...
**Purpose:**  
The `Msr_Budget` measure calculates the most recent available budget value based on the selected week in the visual. If the current budget value is not available, it defaults to the previous budget value.
...
**Purpose:**  
The `MSR_Cumulative Cost` measure calculates the cumulative total of costs incurred up to the selected month in a visual context. This measure is useful for tracking cost trends over time and assessing budget utilization.
...
**Purpose:**  
The `MSR_Cumulative Sales` measure calculates the cumulative sales amount up to the selected month in a Power BI visual. This measure is useful for analyzing sales trends over time, allowing users to see total sales accumulated from the beginning of the year to the selected month.
...
- **DimDate[ShortDate] <= MAX(DimDate[ShortDate]):** Ensures that only dates up to the current maximum date are included in the cumulative total.
...
**Purpose:**  
The `MSR_PROJECT_%_REALIZATION` measure calculates the percentage of sales amount realized against the total sales price for a project. This measure is useful for assessing project performance in terms of revenue realization.
...
**Contextual Notes:**  
- The measure uses the `SUM` function to aggregate the `SalesAmount` from the `vw_Timesheet` table and the `SalesPriceBaseCurrency` from the `tbl_Project` table.
- It includes a conditional check to avoid division by zero, ensuring that the calculation only occurs when the total sales price is not zero.
...
This measure is essential for assessing the financial performance of projects, allowing stakeholders to evaluate profitability effectively. The result is formatted as a general number.
...
**Purpose:**  
The `PercentageCompleteness` measure calculates the completeness percentage of missing hours relative to the expected contracted hours for a given context. It helps in assessing how much of the expected hours have been accounted for in the timesheet data.
...
**Explanation:**  
- **Numerator:** The measure subtracts the `ExpectedContractHoursWeekly` from the total `MissingHours` to determine the difference.
- **Denominator:** The `ExpectedContractHoursWeekly` represents the total expected hours for the week.
- **DIVIDE Function:** This function is used to safely perform the division, returning 0 if the denominator is zero.
...
**Purpose:**  
The `TotalActualHours` measure calculates the total number of hours recorded in the `vw_Timesheet` table. This measure is essential for analyzing actual work hours against contracted hours and other performance metrics.
...
**Purpose:**  
The `TotalContractedHours` measure calculates the total number of contracted hours for all employees by summing the maximum hours per week for each employee. This measure is essential for comparing actual hours worked against contracted hours.
...
This measure is used in conjunction with other measures such as `TotalActualHours` and `TotalHoursDeltaComplete` to assess performance against contracted hours, enabling insights into employee utilization and contract compliance.
...
**Details:**
- **Numerator:** `[TotalActualHours]` - This measure sums the total hours recorded in the `vw_Timesheet` table.
- **Denominator:** `[TotalContractedHours]` - This measure calculates the total contracted hours based on the maximum hours per week for each employee.
- **Division Handling:** The `DIVIDE` function is used to safely handle division, returning 0 if the denominator is 0.
- **Format:** The result is formatted as a percentage with two decimal places (0.00%;-0.00%;0.00%).
...
**Purpose:**  
The `TotalHoursTracked` measure calculates the total hours recorded in the `vw_Timesheet` table. If no hours are recorded, it returns zero instead of a blank value.
...
- The `ISBLANK` function checks if the result of the `SUM` is blank.
- If the sum is blank, it returns the string "Empty"; otherwise, it returns the total hours tracked. 

This measure helps in identifying scenarios where no hours have been logged, providing clarity in reporting and analysis.
...
- `MAXX(DISTINCT(...))`: This part retrieves the maximum distinct value of `HoursPerWeek` for the employee.
...
- **CALCULATE:** Modifies the filter context to compute the maximum hours per week while ignoring filters on `HoursPerWeek` and keeping the context for `EmployeeID`.
- **MAXX & DISTINCT:** Used to find the maximum unique value of `HoursPerWeek`.

This measure is useful for assessing discrepancies between tracked hours and the maximum expected hours, aiding in performance and resource management.
...
**Purpose:**  
The `UTI_TotalBillableHours` measure calculates the total billable hours from the `vw_Timesheet` table, specifically for projects that are either marked as billable or qualify under the "Customer Success Services" category.
...
**Details:**
- **Filtering Logic:** The measure filters the `vw_Timesheet` table to include only those entries where:
  - The project qualifies (`QualifyPrj = 1`) and is billable (`BillablePrj = 1`), or
  - The project name contains "Customer Success Services".
- **Aggregation:** It then sums the `Hours` for the filtered records using `SUMX`, which iterates over the filtered table. 
...
This measure is used to assess the total hours contributed to qualifying projects, facilitating performance analysis and resource allocation.
...
**Details:**
- `_TotalBillableHours`: Retrieves the total billable hours from the `UTI_TotalBillableHours` measure.
- `_TotalHours`: Retrieves the total hours from the `UTI_TOTALHOURS` measure.
...
- If the division results in a blank, it returns 0; otherwise, it returns the calculated ratio. 
