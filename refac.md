...
**Key Columns:**
- **Hours_nd_Percentage**: Represents the summarized values derived from the source column `[Value1]`. It is sorted by the `Hours_nd_Percentage Order`.
- **Hours_nd_Percentage Fields**: A hidden column that groups data based on the source column `[Value2]`, also sorted by `Hours_nd_Percentage Order`.
- **Hours_nd_Percentage Order**: A hidden column that sums values from the source column `[Value3]` and is formatted as a whole number.

**Partitioning and Source:**  
The table is calculated and imported, sourcing data from:
- Total Actual Hours (`vw_Timesheet'[TotalActualHours]`)
- Hours Percentage (`vw_Timesheet'[Msr_HoursPercentage]`)
...
1. It defines a variable `ActiveEmployees` that filters a summarized table (`vw_missing_timesheet`) based on the following criteria:
   ...
   - The total missing hours (`MissingHours`) is less than 0, which indicates that the employee has not reported sufficient hours compared to their contracted hours.
...
This formula first creates a variable `ActiveEmployees` that filters a summarized table of `vw_missing_timesheet` for employees with at least one active status and negative missing hours. It then calculates the sum of missing hours for these filtered employees. 
...
- **[Dax_EmpCount_MissingTS]**: This measure counts the number of employees with missing timesheet entries.
...
1. **Define Date Range:**
   - `_MaxDate`: The maximum date from the `DimDate[ShortDate]` column.
   - `_MinDate`: The minimum date from the `DimDate[ShortDate]` column.

2. **Identify Employees:**
   - `_StartsBeforeMonthEnd`: A table of unique `EmployeeID`s where the `ShortDate` is less than or equal to `_MaxDate`.
   - `_notEndAfterMonthEnd`: A table of unique `EmployeeID`s where the `ShortDate` is greater than or equal to `_MinDate` or is blank, utilizing the relationship between `DimDate[ShortDate]` and `vw_Timesheet[ContractEndDate]`.

3. **Count Active Contacts:**
   - The measure returns the count of rows that are present in both `_notEndAfterMonthEnd` and `_StartsBeforeMonthEnd` using the `INTERSECT` function.

This measure effectively identifies and counts employees whose contracts are active within the defined date range.
...
**Purpose:**  
The `PercentageCompleteness` measure calculates the completeness of hours reported against the expected contracted hours for employees, providing insight into the extent of missing hours.
