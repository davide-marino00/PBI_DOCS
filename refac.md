...
- **DayNumberOfYear** (int64): The numeric representation of the day in the year (1-365).
...
**Data Source:**  
The data is imported from a SQL database located at `devoteamreportingsrv.database.windows.net`, specifically from the `AFASWarehouse` schema.
...
- **Hours_nd_Percentage Order:** This hidden column summarizes values from the source column `[Value3]` using the sum function and is formatted as a whole number.

**Partitioning:**  
The table is calculated and imported, aggregating data from the `vw_Timesheet` view for total actual hours and hours percentage.

**Annotations:**  
The table includes automatic summarization settings and metadata properties for versioning and conceptual binding.
...
**Key Columns:**
- **Code**: (string) Represents the unique code identifier. No summarization is applied.
- **Qualify**: (int64) Represents a numeric qualification value, summarized by sum.
- **Sort**: (string) Represents a sorting attribute. No summarization is applied.
...
**Purpose:** The `lkp_fltr_Employee` table serves as a lookup table for employee information, specifically providing identifiers and names for employees.
...
- **OWN-Sub-ExtT**: (string) Additional categorization related to the unit.
...
**Description:**  
The `tbl_Project` table contains detailed information about various projects, including their identifiers, descriptions, financial details, and status. It serves as a central repository for project-related data, facilitating analysis and reporting.
...
- **TotalContractSum** (decimal): Total financial value of the project contract.
...
This table serves as a comprehensive source for tracking employee hours, project involvement, and contract statuses within the organization.
...
1. It creates a variable `ActiveEmployees` that filters a summarized table derived from `vw_missing_timesheet`. This table includes:
   - Employee names
...
This measure first creates a variable `ActiveEmployees` that filters and summarizes the `vw_missing_timesheet` table to identify employees with negative missing hours and a minimum active employee count of 1. It then returns the sum of the missing hours for these active employees. 
...
**Purpose:**  
The `Msr_ActiveContacts` measure calculates the number of active contacts based on their contract dates within a specified date range.
...
2. It creates a table `_StartsBeforeMonthEnd` that includes distinct `EmployeeID` values from `vw_Timesheet` where the `ShortDate` is less than or equal to `_MaxDate`.
3. It creates another table `_notEndAfterMonthEnd` that includes distinct `EmployeeID` values where the `ShortDate` is greater than or equal to `_MinDate` or is blank, utilizing the `USERELATIONSHIP` function to relate `DimDate[ShortDate]` to `vw_Timesheet[ContractEndDate]`.
...
This measure effectively counts the number of employees whose contracts are active within the defined date range.
...
**Purpose:**  
The `MSR_AVG_HOURLY_RATE` measure calculates the average hourly rate based on sales amounts and total hours worked.
...
1. **Current Week:** It retrieves the current week number from the `DimDate` table.
2. **Previous Budget:** It calculates the previous budget value by filtering the `Msr_SalesPriceBaseCurrency` measure for weeks less than the current week.
3. **Return Value:** It returns the current budget value if it is not blank; otherwise, it returns the previous budget value.
...
This formula first computes the total hours from the `vw_Timesheet` table while considering all selected values for `GroupCat`, `Project`, and `EmployeeName`. It then divides the sum of actual hours by this total value, returning a percentage. If the total value is zero, it returns zero to avoid division errors. 
...
This formula checks if the total sales price in the base currency is not zero. If true, it divides the total sales amount from the timesheet by the total sales price for the project, yielding the realization percentage. 
...
This formula subtracts the `TotalContractedHours` from the `TotalActualHours`, providing a numerical representation of the difference in hours. 
...
**Purpose:**  
The `TotalHoursTrackedMissing` measure is designed to evaluate the total hours tracked in the `vw_Timesheet` table. It specifically checks for the presence of hours and returns a specific output based on that condition.
...
## Page/Report Context
- No applicable page/report-level filters found in the provided JSON.
...
# Visual Documentation for Year Visual
...
- **Visual Type:** Not specified in the provided context.
- **Data Fields Used:**
  - **Projections:** Not specified in the provided context.
  - **Selects:** Not specified in the provided context.
- **Visual-Level Filters:** Not specified in the provided context.
