...
The Power BI solution presented is designed to analyze and visualize timesheet and project-related data, primarily focusing on sales performance, employee hours, and project management metrics. The solution utilizes a complex data model that integrates multiple tables, including `vw_Timesheet`, `tbl_Project`, and `DimDate`, to provide insights into various key performance indicators (KPIs) such as total hours worked, sales amounts, average hourly rates, and project margins. 

Key components of the solution include measures for cumulative sales and costs, as well as calculations for budget utilization and realization percentages. The data model supports various visualizations, including pivot tables that allow users to drill down into specific projects, clients, and employee contributions over time. The use of calculated columns and measures enhances the analytical capabilities, enabling users to track performance against budgets and assess project statuses effectively. Overall, this Power BI solution serves as a comprehensive tool for stakeholders to monitor and optimize project performance and resource allocation.
...
**Purpose:**  
The DimDate table serves as a date dimension, providing various attributes related to dates for use in reporting and analysis.

**Key Columns:**

- **DateKey** (int64): A unique identifier for each date, used for counting.
- **ShortDate** (dateTime): The date in a long date format.
- **DayName** (string): The name of the day (e.g., Monday).
- **DayNumberOfWeek** (int64): The numeric representation of the day within the week.
- **DayNumberOfMonth** (int64): The numeric representation of the day within the month.
- **DayNumberOfYear** (int64): The numeric representation of the day within the year.
- **WeekNumberOfYear** (int64): The numeric representation of the week within the year.
- **MonthName** (string): The name of the month (e.g., January).
- **MonthNumberOfYear** (int64): The numeric representation of the month within the year.
...
**Purpose:** The `lkp_fltr_Employee` table serves as a lookup table for employee information, providing a reference for employee IDs and names.
...
**Data Source:** The table is imported from an Excel workbook located at a specified SharePoint URL, specifically from the "Lookup" sheet. The data is processed to remove null and whitespace entries, promote headers, and change the column types to text.
...
**Data Source:** The table is populated from an Excel workbook located at a specified SharePoint URL, with data transformation steps including filtering, promoting headers, and changing data types.
...
This formula counts the unique `EmployeeID` values where the `MissingHours` are less than zero. The result is formatted as a whole number (0).
...
1. It defines a variable `ActiveEmployees` that filters a summarized table created from `vw_missing_timesheet`. This table includes:
   ...
   - A calculated column for `MinActiveEmp`, which finds the minimum number of active employees.
...
This formula computes the maximum value of `ContractHours` and subtracts the maximum value of `Hours_`, providing a straightforward calculation of the missing hours.
...
1. **Variables:**
   ...
   - `_notEndAfterMonthEnd`: Creates a table of unique `EmployeeID`s where the `ShortDate` is greater than or equal to `_MinDate` or is blank, utilizing a user relationship with `vw_Timesheet[ContractEndDate]`.
...
This measure effectively identifies employees whose contracts are active during the specified period.
...
This formula sums the total sales amount from the `vw_Timesheet` table and divides it by the total hours worked, also sourced from the same table. The result provides the average hourly rate for the tracked time entries. 
...
2. **Previous Budget Calculation:** It calculates the previous budget value by filtering the `DimDate` table to include only weeks prior to the current week and using the `Msr_SalesPriceBaseCurrency` measure.
3. **Return Value:** It returns the current budget value if available; otherwise, it returns the previously calculated budget value.
...
**Purpose:**  
The `Msr_HoursPercentage` measure calculates the percentage of hours worked by a specific group (e.g., by project or employee) relative to the total hours worked across all selected groups.
...
- It first calculates the total hours (`TotalValue`) for all selected categories (GroupCat, Project, EmployeeName).
- Then, it divides the sum of hours for the current context by this total, returning a percentage. If the total is zero, it returns 0. 
...
This formula counts the distinct employee names in the `vw_Timesheet` table while ignoring any filters on the table, ensuring that only employees with a contract status of "Valid" are included in the count. 
...
**Purpose:**  
The `TotalHoursTrackedMissing` measure is designed to evaluate the total hours tracked in the `vw_Timesheet` table. It provides a way to identify whether the tracked hours are present or if they are missing.
...
* `Sales`.[`ProjectKey`] -> `Project`.[`ProjectKey] (Type: manyToOne`]
* Active: true (Could not parse format)
...
* CrossFilter: bothDirections) (Could not parse format)
...
No calculated columns discovered or documented.
